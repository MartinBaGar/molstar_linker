// src/config.ts
import { TargetDefinition, Preset, ExtensionSettings } from './types.js';

export const AppConfig: {
  RepSchema: Record<string, { label: string; params: Record<string, any> }>;
  targets: TargetDefinition[];
  presets: Record<string, Preset>;
  getDefaults: () => ExtensionSettings;
} = {
  RepSchema: {
  ...
  getDefaults: function(): ExtensionSettings {
    const defaults: ExtensionSettings = {
      canvas_color: "#ffffff",
      camera_json: ""
    };
    this.targets.forEach((t: TargetDefinition) => {
