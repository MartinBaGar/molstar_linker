...
async function bootWorkspace(rawUrl, safeFormat) {
    ...
    try {
        if (!isSafeUrl(rawUrl))
            throw new Error("Access to this local or restricted network address is blocked for security.");
...
function spawnIframe(dataUri, format, sourceUrl) {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!rawUrl) {
        if (loadingDiv)
            loadingDiv.innerText = 'Empty Workspace. Drag and drop a file to begin.';
        return;
    }
